...
\textbf{要点}
\begin{itemize}
  \item P3 窄路密集锥桶场景，18 个路锥沿车道两侧布置成通行带。
...
\begin{itemize}
  ...
  \item 复杂度从 $O(2^k)$ 指数级压到 $O(k\log k)$ 对数线性。以 P3 窄路锥桶为例，组规模 $k\!=\!18$，原本 $2^{18}\!\approx\!26$ 万次组合判断，MIKU 按排序一步出解。
\end{itemize}
...
\begin{itemize}
  ...
  \item 压力 P1--P4\quad{}Baseline 通过率 \MPressureBaselinePassCount/\MPressureScenarioTotal 全灭，MIKU \MPressureMikuPassCount/\MPressureScenarioTotal 全通；平均速度 \MPressureBaselineAvgV$\to$\MPressureMikuAvgV\,m/s，QP 总耗时 \MPressureBaselineQpTotal$\to$\MPressureMikuQpTotal\,ms，降 84.3\%。
  \item 可比 C1--C4\quad{}两者都能走完，但 MIKU 把 QP 总耗时从 \MComparableBaselineQpTotal\,ms 降到 \MComparableMikuQpTotal\,ms，降 66.6\%。
...
\textbf{要点}
\begin{itemize}
  \item 六组消融变体是减法消融 leave-one-out 口径，从 MIKU 全量依次关闭单个组件。
  \item M0 Apollo Baseline\quad{}\AblationScoreMZero\,分，通过 \AblationMZeroPassCount/\AblationMZeroScenarioTotal。
  \item M1 MIKU w/o C1（无 $\tau$ 投影）\quad{}\AblationScoreMOne\,分，\AblationMOnePassCount/\AblationMOneScenarioTotal；动态行人在路径阶段被关成静态快照，P1/P2 均失败。
  \item M2 MIKU w/o C2--C3（无组级决策）\quad{}\AblationScoreMTwo\,分，\AblationMTwoPassCount/\AblationMTwoScenarioTotal；P4 密集施工逐障碍物贪心与水马方向对冲，$s_{end}$ 仅 \AblationMTwoScFourSEndPct\% 即停车。
  \item M3 MIKU w/o C4（统一 $\delta$）\quad{}\AblationScoreMThree\,分，\AblationMThreePassCount/\AblationMThreeScenarioTotal；P3 窄路锥桶统一缓冲压死通行带。
  \item M4 MIKU w/o C5（无走廊注入）\quad{}\AblationScoreMFour\,分，\AblationMFourPassCount/\AblationMFourScenarioTotal；C5 接口在当前压力场景未激活，与 M5 仅差 $\AblationMarginMFourMFive$\,分。
  \item M5 MIKU 全量\quad{}\AblationScoreMFive\,分，\AblationMFivePassCount/\AblationMFiveScenarioTotal。
  \item 结论是路径阶段 C1/C2--C3/C4 四组件互补，任缺其一即在 P1--P4 中某一场景失败；C5 走廊注入的定量贡献需要多动态冲突点场景补测。
\end{itemize}
\textbf{过渡}\quad{}下面看耗时。\\
...
\textbf{要点}
\begin{itemize}
  \item P2 行人加停车差距最大，Baseline QP 总耗时 \MTwoBaselineQpTotal\,ms，MIKU 只要 \MTwoMikuQpTotal\,ms，加速 24 倍。
  \item C2 行人加停车可比\quad{}\MSixBaselineQpTotal$\to$\MSixMikuQpTotal\,ms，加速 16.6 倍。
