...
实现上仍以保守包络处理链式连通分量，保证安全可行但不声称全局最优。需要说明，式\eqref{eq:time_varying_opt}的极大极小解是固定单一分割约束下的受限最优，而非允许分割沿 $s$ 变化的无约束最优；逐截面最优的分割点 $p^{*}(s)$ 一般随 $s$ 变化，但单条连续偏移曲线无法实现分割沿程切换，故本文以单分割受限最优为工程上可实现的目标，定理\ref{thm:max_gap}的逐截面最优性在此约束下退化为对最薄截面的保证。式\eqref{eq:time_varying_opt}本身可由定理\ref{thm:max_gap}直接得到，由各截面 $W(p,s)=g_p(s)$，固定分割 $p$ 的整段通行带宽度为 $\min_s g_p(s)$，在 $k+1$ 个候选分割中取最大即得 $W^{*}$，故其为指派固定前提下的最优。选定最优分割 $p^{*}$ 后，把 SL 通行性沿 $\tau(s)$ 抬升至 ST 平面，得到时空可通行走廊
