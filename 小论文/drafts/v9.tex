...
有了 $\tau(s)$，每个动态障碍物的横向占据便取自其在自车到达时刻的预测位置。对障碍物 $i$，在 Prediction 模块给出的未来 $6$\,s 预测轨迹中查询时刻 $\tau(s)$ 的位置并线性插值，投影至参考线 Frenet 坐标得 $(s_i(\tau), l_i(\tau))$，结合几何尺寸得到时变 SL 边界 $[l_i^{-}(\tau(s)), l_i^{+}(\tau(s))]$；若该时刻障碍物纵向位置与 $s$ 的距离超过其纵向尺寸一半，则标记其在 $s$ 处不活跃。预测视野之外保守取最后预测位置不变，即路径阶段时变投影以 Prediction 输出的 $6$\,s 预测轨迹为源，超出该视野的部分按上述兜底延拓，其与速度阶段 $7$\,s ST 视野的衔接由该越界保守延拓处理。当前实现取概率最高的一条轨迹，扩展到多轨迹并集可降低漏判风险但查询次数线性增加。第\ref{sec:method_threat}节的横向重叠率因子同样改用该时变投影，即以 $f_{overlap}(i, \tau(s))$ 评估动态障碍物，静态障碍物仍取 $t=0$ 观测位置。
...
实现上仍以保守包络处理链式连通分量，保证安全可行但不声称全局最优。需要说明，式\eqref{eq:time_varying_opt}的极大极小解是固定单一分割约束下的受限最优，而非允许分割沿 $s$ 变化的无约束最优；逐截面最优的分割点 $p^{*}(s)$ 一般随 $s$ 变化，但单条连续偏移曲线无法实现分割沿程切换，故本文以单分割受限最优为工程上可实现的目标，定理\ref{thm:max_gap}的逐截面等式沿固定分割复合后，整段宽度由最薄截面 $\min_s g_p(s)$ 决定，受限最优即在 $k+1$ 个候选分割上取其最大，约束施加于可行解集合而非削弱定理结论本身。式\eqref{eq:time_varying_opt}本身可由定理\ref{thm:max_gap}直接得到，由各截面 $W(p,s)=g_p(s)$，固定分割 $p$ 的整段通行带宽度为 $\min_s g_p(s)$，在 $k+1$ 个候选分割中取最大即得 $W^{*}$，故其为指派固定前提下的最优。选定最优分割 $p^{*}$ 后，把 SL 通行性沿 $\tau(s)$ 抬升至 ST 平面，得到时空可通行走廊
...
该操作只改写 $\mathbf{x}^{ub}$ 中对应分量的值，保持 $\mathbf{H}$、$\mathbf{A}_{eq}$ 与稀疏模式完全不变，对纵向占据范围大的障碍物可两端采样把 $\mathcal{T}$ 扩为 $2|\mathcal{D}|$ 个约束。求出的最优速度曲线满足 $(s^{*}(t_j), t_j) \in \Omega$ 对所有 $j=0,1,\ldots,K$ 成立。工程上走廊约束以 \texttt{STDrivableBoundary} 这一 ST 可行驶边界结构承载。该结构在原生 Apollo 中由 \texttt{STBoundsDecider} 这一 ST 边界生产环节在含其的 pipeline 中生产，其唯一原生消费者为 \texttt{PathTimeHeuristicOptimizer} 的动态规划代价项，且该消费受 \texttt{use\_st\_drivable\_boundary} 开关门控、默认关闭。本文接入的 \texttt{LaneFollowPath} 路径 pipeline 中并不含 \texttt{STBoundsDecider}，该结构本就闲置，速度二次规划亦不读取它。本文重新利用这一既有闲置数据结构。由 \texttt{LaneFollowPath} 的 \texttt{BuildMikuSTDrivableBoundary} 依据路径阶段的时变投影重建其内容，写入 \texttt{ReferenceLineInfo} 持有的 \texttt{StGraphData} 的 \texttt{STDrivableBoundary} 字段。该字段随数据总线传至速度阶段，其间速度边界决策仅刷新 ST 边界而不清空它，故路径阶段的写入得以存活至速度阶段被读取。读取后在 \texttt{PiecewiseJerkSpeedOptimizer} 处按式\eqref{eq:tighten_ub}收紧上界，从而把走廊约束接入原本不经过该结构的速度二次规划。上述复用以 \texttt{use\_st\_drivable\_boundary} 默认关闭为前提，本文接入运行于该开关关闭的生产配置下；该开关若被开启，则 \texttt{PathTimeHeuristicOptimizer} 亦会读取经本文重写的该字段，故复用的无副作用性限定在默认配置之内。

走廊上界与 \texttt{SpeedBoundsDecider} 的原有输出在 QP 层取最小值完成双源融合，此处 \texttt{SpeedBoundsDecider} 指 pipeline 末次即 \texttt{SpeedDecider} 之后的 FINAL 调用所产 ST 边界，亦即与速度二次规划直接衔接者
...
复杂度可逐段相加。到达时间估计 $O(K)$，$K$ 为路径离散点数；时变投影只在障碍物活跃的纵向区间上对其查询预测轨迹，单障碍物跨其活跃区间的累计采样数为 $O(N_s)$，而在障碍物空间密度有界即任一截面同时活跃的障碍物数为 $O(1)$ 的前提下，全部障碍物跨各自活跃区间的累计采样数 $\sum_i N_s = O(K)$，时变投影合计 $O(K)$；扫描线排序 $O(n\log n)$、扫描 $O(n)$；组内排序合计 $\sum_j O(k_j\log k_j)\leq O(n\log n)$，由 $k\log k$ 的凸性与 $\sum_j k_j = n$ 保证；间隙计算与边界构建 $O(K)$；交互密度因子的邻域统计经空间桶索引摊还为 $O(n)$，在密度有界前提下不抬阶，详见 §\ref{sec:thy_complexity}。整体为 $O(n\log n + K)$，$K$ 为路径或时间离散规模，此阶在障碍物空间密度有界的前提下成立，由扫描线分桶的摊还分析给出，最坏退化情形见 §\ref{sec:thy_complexity}。相比之下 Baseline 逐障碍物贪心在每个离散位置重算全部障碍物，为 $O(nK)$。需说明算法 \ref{alg:miku} 内层循环虽按障碍物书写，实现上仅遍历扫描线在该截面维护的活跃集而非全部 $n$ 个障碍物，在密度有界下其累计为 $O(K)$，故整体不退化为 $O(nK)$。单截面层面，MIKU 将暴力枚举的 $O(2^k)$ 降为 $O(k\log k)$，在满足 Apollo 实时预算的同时把方向组合的搜索做到完备。三个组件由此合成一条计算代价几乎不增的显式约束传递通道，第\ref{sec:experiment}节将在仿真场景上验证其通行能力与开销。
...
需要强调，$\tau(s)$ 由上周期速度曲线查表或二阶运动学模型估计给定，在路径层是已知输入而非优化变量，命题 \ref{prop:decoupled_infeasible} 因此不涉及对 $\tau(s)$ 的联合优化。该命题揭示解耦失效的根源在于时间维度缺失导致占用区域被过度膨胀，由此引出在解耦接口处显式传递时间约束的核心思路。它给出的是充分条件：满足式 \eqref{eq:thy_infeasible_cond} 必然触发解耦阻塞，但解耦阻塞也可能源于后文讨论的逐障碍物贪心决策的虚假阻塞，二者需加区分。需补充其覆盖范围：式 \eqref{eq:thy_infeasible_cond} 刻画单个动态障碍物时间扫掠并集封锁整段可行区间这一类时间维封锁；多个障碍物各自只封一部分、其空间并集才覆盖 $\mathcal{B}_{W}(s)$ 的联合封锁，在时不变并集投影下由定理 \ref{thm:max_gap} 的 $\max_p g_p < W_{\text{ego}}$ 判据处理，即时间维单障碍封锁归本命题、空间维联合封锁归定理 \ref{thm:max_gap}，二者拼出对解耦失效检测侧的完整覆盖。恢复侧与此对称，单障碍时间封锁经引入 $\tau(s)$ 恢复横向通行性如本命题逆向所示，多障碍空间联合封锁则由定理 \ref{thm:max_gap} 的最大间隙分割在通行带宽度恢复至车宽时给出可行带，两类封锁的恢复条件分别由命题 \ref{prop:feasibility_recovery} 的走廊非空判据与最大间隙判据承载。
...
证明。规则 A 下 $l^{+}(\mathbf{d}) < l^{-}(\mathbf{d})$ 对任意 $\mathbf{d}$ 成立，无可行分割，等价于最优宽度 $W^{*}=0$。规则 B 中若分配 $d_i = R$，则 $l^{+}(\mathbf{d}) \leq u_i < l_{road}^{-} \leq l^{-}(\mathbf{d})$，通行带宽度非正，故 $d_i = L$ 为唯一最优选择；被强制左绕的障碍物等效于把道路右边界内收至其 $v_i$ 作为全局下界并入 $\max$ 运算，将其移出候选列表后剩余 $k-1$ 个障碍物按原序重新连续编号，$k-1$ 个候选分割的相对间隙结构原样成立；越界障碍物的 $v_i$ 抬高全局下界基线 $l_{road}^{-}\to\max(l_{road}^{-},v_i)$ 后，剩余障碍物因未越界仍满足 $u_j\geq l_{road}^{-}$，端部间隙 $g_0$ 相应以抬高后基线计算，定理前提不破坏。规则 C 与规则 B 对称。三条规则预处理后，剩余障碍物全部满足 $u_j \geq l_{road}^{-}$ 且 $v_j \leq l_{road}^{+}$，定理 \ref{thm:max_gap} 的前提在 $k-1$ 规模上重新满足。$\hfill\Box$
...
走廊 $\Omega$ 由 $\tau(s)$ 的左下沿与若干水平禁行切片组合而成，其中禁行切片对应某纵向位置处最优间隙不足车宽时整列被排除的情形，因此 $\Omega$ 作为集合一般是非凸的。凸性来源不在 $\Omega$ 本身，而在其沿离散时间步的逐时间步线性投影。注入过程对每个关键位置仅按到达时间对相应时间步的纵向上界做一次取最小值的收紧，即把走廊约束化为速度变量的盒式上界，与 \texttt{SpeedBoundsDecider} 给出的 $ST$ 边界取最小值融合。该盒式约束与 Baseline 速度链路经 \texttt{PathTimeHeuristicOptimizer} 动态规划粗搜索、\texttt{SpeedDecider} 标定让行或超车、再由 \texttt{SpeedBoundsDecider} 产出的线性约束在二次规划结构上一致，因此 \texttt{PiecewiseJerkSpeedOptimizer} 的目标矩阵、等式约束与稀疏模式完全不变，仅上界向量的若干分量被收紧。
...
证明。先证可行域非空。需注意速度二次规划的可行域并非各时间步盒约束的笛卡尔积，而是分段加加速度运动学等式约束所定义的仿射子空间与盒约束之交 $\{\mathbf{x}\mid \mathbf{A}_{eq}\mathbf{x}=\mathbf{b}_{eq},\ \mathbf{x}^{lb}\le\mathbf{x}\le\mathbf{x}^{ub}\}$，等式约束把相邻时间步的状态强耦合，因此逐时间步盒区间非空并不直接蕴含该交集非空，尚需一步可达性论证。由 $\min_s g_{p^{*}}(s) \geq W_{\text{ego}}$，分割 $p^{*}$ 在每个截面均给出宽度不小于车宽的通行带，其曲率可行性由路径二次规划的曲率约束处理，本命题在给定路径几何上讨论速度可行性。沿该路径，走廊注入仅按到达时间收紧位置上界、不改写下界 $s_j^{lb}=s_j^{lb,\mathrm{SBD}}$，几何含义为要求自车不早于 $\tau(s)$ 到达而从不要求其提前到达。可达性论证的关键是从原始盒内的可行解出发，沿减速或保持方向构造一条等待型剖面，使其位置分量逐时间步不超过收紧后的上界。当注入走廊只收紧上界而不引入强制自车在某时刻前越过某纵向位置的正下界时，这样的等待型剖面不破坏运动学可达性，其位置分量逐时间步既不超过收紧后的上界、又不低于未被改写的原始下界，由前提 $s_j^{ub} \geq s_j^{lb}$ 知该剖面落入收紧后的盒并满足运动学等式约束，可达性即闭合，亦即在该常见情形下命题前提中的收紧盒可行解可由原始盒可行解构造性给出。这一条件在本文接入的 \texttt{LaneFollowPath} 速度链路中成立，其 \texttt{SpeedBoundsDecider} 纵向下界为不后退型边界，超车与让行决策由 \texttt{SpeedDecider} 标定后体现为 $ST$ 上界的收紧而非正下界，故等待方向不被封堵；即便某条链路确实引入了限制等待方向的正下界，命题已将走廊收紧后盒内满足运动学等式约束的可行解之存在直接列为前提，可行域非空性与最优解的存在性由该前提而非下界性质保证，结论对下界正负不敏感。需明确二者的分工，沿减速或保持方向构造等待型剖面给出的是该前提在不后退型下界这一常见情形下被满足的一个充分构造，而结论本身仅依赖收紧盒可行解存在前提，故对下界正负不敏感。由此可行域 $=$ 仿射子空间 $\cap$ 盒非空。再证最优解存在。该交集为有限个闭区间盒与仿射子空间之交，仿射集与盒均凸，其交仍凸；又位置分量由收紧后有限上界 $s_j^{ub}$ 与下界 $s_j^{lb}$ 双侧框定、速度与加速度分量由各自盒约束框定，决策变量整体有界，故可行域为非空有界闭凸集。连续二次凸函数在非空有界闭凸集上必取得最小值，故增强后的速度二次规划存在最优解。$\hfill\Box$

命题 \ref{prop:feasibility_recovery} 的前提需明确界定。其一，结论限定在离散时间网格上，即逐时间步线性投影后的有限维盒式约束，而非对非凸集合 $\Omega$ 整体声称凸性；盒约束交集的凸性自动成立，真正进入前提的是决策变量的有界性，由速度与加速度盒约束保证。其二，可行域非空依赖两层条件，一层是几何条件 $\min_s g_{p^{*}}(s) \geq W_{\text{ego}}$ 与逐关键位置的 $s_j^{ub} \geq s_j^{lb}$，保证收紧后的盒区间逐步非空，另一层是走廊收紧后的盒内存在满足运动学等式约束的可行解，已作为命题前提显式列出，保证盒与仿射子空间之交非空。前一层由场景几何决定，后一层在不后退型下界下由原始盒可行性经等待型构造天然导出，二者共同支撑非空性而非以下界正负为基石。当障碍物在自车到达前无法让出足够空间，即不存在满足几何条件的分割时，收紧后的上界与下界冲突、走廊为空，对应真实需要等待或停车的物理情形，不属于可恢复范畴。该几何条件与命题 \ref{prop:decoupled_infeasible} 的并集覆盖条件恰成失效与恢复的对偶。其三，走廊的禁行切片，即某 $s_k$ 处最优间隙不足车宽而整列被排除的情形，经 blocked-then-trim 由路径几何长度隐式传达，把超出 $s_k$ 的纵向区间移出 QP 决策变量，故本命题的凸性与可行性分析只需在被截断后的有效区间上讨论 $\tau(s)$ 左下沿的上界收紧。其四，走廊由 $\tau(s)$ 构造，而 $\tau(s)$ 又依赖速度规划结果，二者构成循环依赖。本文借助 Apollo 每个规划周期的周期性重规划以单步延迟方式处理，每周期用上周期速度输出更新 $\tau(s)$，与解耦框架的现有做法一致；该近似的理论收敛性分析作为开放问题，本文不予声称。
...
本节在Apollo 11.0生产级代码的工程接入之上，用数值仿真量化显式约束传递通道带来的通行能力、轨迹质量与求解开销变化。实验给出四类结果，依次是 8 场景的通行成功率与速度对照、可比场景下的求解效率对照、5 个机制的组件消融，以及威胁度权重的灵敏度分析，另含平台与场景设置、局限性讨论各一节。本节末尾单列局限性，主动声明本工作在验证边界上的不足。
...
每个场景计算六项指标，覆盖通行能力、轨迹舒适性与求解开销三类。通行能力由纵向到达率$P_{\mathrm{pass}}$与平均速度$\bar v$度量，其中$P_{\mathrm{pass}}$以仿真终点满足$s_{\mathrm{end}} \geq s_{\max} - 1$\,m为判据。该$1$\,m容差用于吸收离散终点的数值余量；因压力场景中Baseline触发停车降级、其终点里程远低于阈值，而MIKU完整到达终点，二者在$s_{\mathrm{end}}$上相差一整段可通行里程，故0/4与4/4的通行结论对该容差在合理区间内的取值不敏感。需要明确该判据的语义边界，它只检验自车是否纵向行驶到终点，不含碰撞检测、几何侵入检测或最小安全间距校验，因此通过指的是纵向到达终点而非已验证的安全通行。鉴于MIKU的差异化裕度组件会把低威胁障碍物的安全裕度压至下限$0.10$\,m以释放通行空间，本判据无法排除贴近障碍物通过的情形，对应的最小安全间距量化评估列为后续工作，详见局限节。轨迹舒适性由最大纵向加速度幅值$|a_x|_{\max}$、最大横向加速度幅值$|a_y|_{\max}$与最大jerk幅值$|j|_{\max}$度量，其中横向加速度由路径曲率诱导，$a_y = v^2 \kappa$。求解开销由路径QP与速度QP合计的总耗时$t_{\mathrm{qp}}$度量。后续各结果表均沿用这一口径。
...
可比场景的轨迹质量对照进一步说明约束前置的作用。MIKU把避让决策提前至路径阶段后，速度阶段不再需要被动急刹，4 个可比场景的舒适性指标对照如表\ref{tab:exp_comfort}所示。在C1、C2两个动态场景下，MIKU的纵向加速度峰值与jerk峰值显著低于Baseline，其中C1的jerk峰值由8.19\,m/s$^3$降至1.62\,m/s$^3$，纵向行驶更平顺；C3、C4两个场景两者纵向加速度与jerk峰值逐项相同且同处数值地板，C3为$|a_x|=0.14$、$|j|=0.14$，C4为$|a_x|=0.11$、$|j|=0.11$，两个量纲不同的指标取到相同两位小数并非抄录错误，而是这两个场景纵向轨迹近乎匀速、平顺指标贴近零地板所致，故C3、C4在舒适性维度不承载区分信息。差异化裕度并未以牺牲纵向舒适性为代价换取通行空间。需如实指出，MIKU因在路径阶段主动绕行而引入了一定的横向加速度，C1、C2的横向加速度峰值由Baseline的近零分别升至1.07与1.45\,m/s$^2$，C4场景MIKU横向加速度峰值1.87\,m/s$^2$与Baseline的1.93\,m/s$^2$相当，反映纵向平顺的改善部分以横向机动换取，但所涉横向加速度量级有限，其峰值1.07至1.87\,m/s$^2$未超出Baseline自身在本组场景中产生的横向加速度水平，C4中Baseline即达1.93\,m/s$^2$。需说明本工作并未锚定外部乘坐舒适阈值，此处仅就本组场景内两者的相对量级作对照，不就绝对舒适性下结论。因此本工作的纵向平顺改善实际由C1、C2两个动态可比场景体现，二者均派生自Baseline失效构型，C3、C4因纵向近匀速不构成平顺区分的证据。换言之，纵向平顺改善这一正面结论的实证基础为 C1、C2 两个派生自失效构型的场景，样本数 $n=2$ 且非独立采样，其向独立常规工况的推广有待补充。
...
QP求解总耗时的对照如表\ref{tab:exp_qp}所示。需要先说明效率比较的口径。在压力场景P1--P4中Baseline路径阶段判定阻塞后回退至FallbackPath停车，其速度求解针对的是一个短路径或停车的降级问题，与MIKU在完整通行路径上的求解非同一规模，记录耗时不构成两种算法的对等效率比较依据。因此本工作的求解效率结论仅基于C1--C4可比场景，该组两者均完成完整求解，耗时可比。
...
\end{equation}
其中$n_i$为子指标的归一化值，$w_d$为维度权重。全局得分对所有压力场景再做几何平均并放大至百分制，若任一场景因硬门得零分则退回算术平均并乘以一个保守经验折减系数 $0.5$。几何平均的否决性使单项失效直接拉低总分，而非被其他高分项掩盖。需说明该综合评分仅作变体间的辅助排序，本节消融的主结论建立在无自由参数的客观量即各变体的通过数之上，含零分场景的 M1 至 M3 其相对次序由通过数决定，折减系数 $0.5$ 只影响这三者的绝对分值而不改变其低于全通过变体的排序结论，故主结论对该系数取值不敏感。
...
综合评分在很大程度上反映的是通过率差异而非通过场景内的质量差异，这是硬门与几何平均否决性的直接结果。需要分两层说明权重的作用。其一，跨通过数的粗分层结论，即 2/4、3/4、4/4 三档之间的高下，主要来自安全硬门的否决性，对四维权重0.30/0.30/0.25/0.15的具体取值并不敏感，M1、M2、M3 因在关键压力场景失败被硬门归零、几何平均随之退化，全局得分均落到38分以下，这一粗分层不随权重取值改变。其二，同通过数变体之间的相对排序则确由权重决定，M2与M3同为3/4，其37.13对34.50的2.63分之差落在评分的测时噪声与权重取值敏感量级内，不承载机制级结论，本工作不据此对二者孰更重要下强结论；M2与M3的机制必要性完全交由其各自的失效场景这一无自由参数的事实链承载。对走廊注入未触发活跃边界的M4与完整MIKU之间，二者在路径几何、速度几何与通行三个质量维度上逐点相同，0.82分之差落在计算开销维度单次墙钟测时的噪声量级内，不反映质量差异。权重取值参照Apollo默认代价项中跟踪、平顺与计算开销的相对量级设定。结合各变体的失效场景可以定位机制的必要性。关闭时变投影的M1在单行人横穿与行人加停车两个动态压力场景失败，动态行人退化为$t=0$快照占据车道，路径阶段被错误约束至当前位置而强制减速等待。这说明一旦选择把动态障碍物纳入路径阶段，就必须配合时变投影，否则反而产生副作用。关闭分组与间隙决策的M2在单车道封闭导流场景失败，该场景的多个静态障碍物构成单一大型连通分量，逐障碍物贪心决策下入口与维持段各障碍物独立选择避让方向，产生左右边界冲突而触发blocked-then-trim停车。关闭差异化裕度的M3在窄路交通锥场景失败，双侧锥间隙仅略大于车身宽度，统一额外缓冲对小尺寸锥桶过度保守，把通行带压缩至不可行。由此，时变投影与分组间隙决策共同决定动态与大型连通分量场景的通行成功率，差异化裕度决定窄路锥列下的可行性。还需指出证据厚度的限度，上述机制必要性的事实链各由单个失效场景（M2、M3）或两个失效场景（M1）支撑，消融组规模为 P1--P4 共 4 个压力场景，故每项机制的关闭即失效本质上是单点或双点事件而非跨多构型复现的稳健结论，其向更多障碍物构型的必要性复现有待补充。
...
枚举--博弈 & 是 & 枚举组合后合作博弈选优 & $O(m_p m_v)$ & 选优非证明 \\
...
在 Python 复现的 8 个仿真场景上的对比表明，4 个压力场景中 Baseline 与 MIKU 的通过率分别为 0/4 与 4/4，汇总平均速度由 3.53\,m/s 提升至 4.57\,m/s，相对提升 29.3\%，该提升主要源于压力场景中 Baseline 进入 \texttt{FallbackPath} 停车降级拉低了均值，可比场景下两者速度持平，并非常规巡航工况下的同等提升。消融、灵敏度与逐场景求解耗时的完整结果见第 \ref{sec:experiment} 节，其中走廊注入组件因当前压力场景未激活其活跃边界而在本组消融中贡献不显，最小安全间距、统计检验与走廊激活场景验证等局限于下节集中说明。
