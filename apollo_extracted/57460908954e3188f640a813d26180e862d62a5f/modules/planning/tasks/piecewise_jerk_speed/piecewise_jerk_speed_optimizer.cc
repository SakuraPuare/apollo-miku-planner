...
Status PiecewiseJerkSpeedOptimizer::Process(const PathData& path_data,
  ...
  // [新增] 若 STDrivableBoundary 已设置，用走廊约束覆盖独立 boundary 约束
  const auto& drivable = st_graph_data.st_drivable_boundary();
  if (drivable.st_boundary_size() > 0) {
    // Rebuild s_bounds from corridor, interpolating to match num_of_knots.
    const int corridor_size = drivable.st_boundary_size();
    s_bounds.clear();
    s_bounds.reserve(num_of_knots);
    for (int i = 0; i < num_of_knots; ++i) {
      const double curr_t = i * delta_t;
      // Find the corridor point bracket that contains curr_t.
      double s_lower = 0.0;
      double s_upper = total_length;
      if (corridor_size == 1) {
        s_lower = drivable.st_boundary(0).s_lower();
        s_upper = drivable.st_boundary(0).s_upper();
      } else {
        // Linear interpolation between adjacent corridor points.
        for (int j = 0; j < corridor_size - 1; ++j) {
          const double t0 = drivable.st_boundary(j).t();
          const double t1 = drivable.st_boundary(j + 1).t();
          if (curr_t >= t0 && curr_t <= t1) {
            const double alpha =
                (t1 > t0) ? (curr_t - t0) / (t1 - t0) : 0.0;
            s_lower = drivable.st_boundary(j).s_lower() * (1.0 - alpha) +
                      drivable.st_boundary(j + 1).s_lower() * alpha;
            s_upper = drivable.st_boundary(j).s_upper() * (1.0 - alpha) +
                      drivable.st_boundary(j + 1).s_upper() * alpha;
            break;
          }
        }
        // If curr_t is beyond the corridor end, use the last corridor point.
        if (curr_t > drivable.st_boundary(corridor_size - 1).t()) {
          s_lower = drivable.st_boundary(corridor_size - 1).s_lower();
          s_upper = drivable.st_boundary(corridor_size - 1).s_upper();
        }
      }
      s_upper = std::fmax(s_upper, s_lower + kEpsilon);
      s_bounds.emplace_back(s_lower, s_upper);
    }
    AINFO << "s_bounds overridden by STDrivableBoundary corridor ("
          << corridor_size << " corridor points).";
  }
