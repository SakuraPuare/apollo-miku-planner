...
Status SpeedBoundsDecider::Process(
  ...
  // [新增] 多障碍物时空走廊优化
  if (config_.use_multi_obstacle_corridor() && !boundaries.empty()) {
    // Collect obstacle pointers for threat assessment.
    std::vector<const Obstacle *> obstacle_ptrs;
    obstacle_ptrs.reserve(boundaries.size());
    for (auto *obstacle : path_decision->obstacles().Items()) {
    ...
    const auto threat_info =
        assessor.Assess(boundaries, obstacle_ptrs, init_point);

    STCorridorGenerator corridor_gen;
    const auto corridor =
        corridor_gen.Generate(boundaries, threat_info, init_point,
                               total_time_by_conf, path_data_length);

    if (!corridor.empty()) {
      std::vector<std::tuple<double, double, double>> s_boundary;
      s_boundary.reserve(corridor.size());
      for (const auto &pt : corridor) {
        s_boundary.emplace_back(pt.t, pt.s_lower, pt.s_upper);
      }
      if (!st_graph_data->SetSTDrivableBoundary(s_boundary,
                                                 {} /*v_obs_info*/)) {
        AWARN << "SetSTDrivableBoundary failed; falling back to default bounds.";
      } else {
        AINFO << "Multi-obstacle corridor set with " << corridor.size()
              << " points.";
      }
    }
  }
