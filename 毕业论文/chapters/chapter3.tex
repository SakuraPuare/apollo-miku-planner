...
\item 以锚点位置为约束，通过离散点偏差最小化QP优化生成平滑的参考线。
...
两阶段之间的信息传递是单向的，路径阶段输出\texttt{PathData}，包含离散路径点和Frenet坐标系路径，速度阶段在此路径的基础上构建ST图并进行速度优化。在Apollo的Task流水线中，各Task按配置顺序执行，不进行回溯迭代。LANE\_FOLLOW场景的完整Task流水线配置如算法\ref{alg:lane_follow_pipeline}所示。

\begin{algorithm}[htbp]
\caption{LANE\_FOLLOW场景Task流水线}
\label{alg:lane_follow_pipeline}
\KwIn{参考线、障碍物列表、自车状态}
\KwOut{规划轨迹（路径+速度）}
\textcolor{gray!30!black}{\textit{// 路径阶段}}\;
依次尝试 LaneChangePath、LaneFollowPath、LaneBorrowPath、FallbackPath \;
执行 PathDecider（路径决策）\;
执行 RuleBasedStopDecider（规则停车决策）\;
\textcolor{gray!30!black}{\textit{// 速度阶段}}\;
执行 SpeedBoundsDecider（先验速度边界构建）\;
执行 PathTimeHeuristicOptimizer（DP启发式搜索）\;
执行 SpeedDecider（速度决策）\;
执行 SpeedBoundsDecider（最终速度边界构建）\;
执行 PiecewiseJerkSpeedOptimizer（QP速度优化）\;
\KwRet 规划轨迹 \;
\end{algorithm}
