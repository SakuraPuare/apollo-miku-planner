...
\begin{figure}[htbp]
\centering
\adjustbox{max width=\textwidth}{\input{../图片/fig_planning_dataflow}}
...
\begin{figure}[htbp]
\centering
\adjustbox{max width=\textwidth}{\input{../图片/fig_scenario_hierarchy}}
...
\begin{figure}[htbp]
\centering
\adjustbox{max width=\textwidth}{\input{../图片/fig_planning_arch}}
...
\begin{figure}[htbp]
\centering
\adjustbox{max width=\textwidth}{\input{../图片/fig_ref_smooth}}
\caption{参考线平滑过程示意图}
...
\begin{figure}[htbp]
\centering
\adjustbox{max width=\textwidth}{\input{../图片/fig_path_speed_decouple}}
...
\textbf{DecidePathBounds()}\quad{}调用\texttt{PathBoundsDeciderUtil}工具类，沿参考线在每个离散纵向位置$s$上聚合车道几何、静态障碍物投影和借道决策，构建横向可行走廊$[l^-(s), l^+(s)]$作为优化问题的硬约束。

\textbf{OptimizePath()}\quad{}以$l(s)$的分段jerk最小化为目标，在离散化网格上将平滑性、参考线偏离和边界约束组织为稀疏二次规划，调用OSQP求解得到横向位置、横向速度和横向加速度三元组序列，输出候选路径。

\textbf{AssessPath()}\quad{}调用\texttt{PathAssessmentDeciderUtil}对候选路径进行有效性筛查，包含终点可达性、与静态障碍物的碰撞检测以及通行带是否被完全阻塞等判定。若\texttt{LaneFollowPath}评估失败，则按优先级依次尝试\texttt{LaneBorrowPath}和\texttt{FallbackPath}，每个Task内部同样执行上述三步。
...
stage {
  ...
  task { name: "LANE_FOLLOW_PATH"            type: "\texttt{LaneFollowPath}" }
  task { name: "LANE_BORROW_PATH"            type: "LaneBorrowPath" }
  task { name: "FALLBACK_PATH"               type: "FallbackPath" }
  task { name: "PATH_DECIDER"                type: "\texttt{PathDecider}" }
  task { name: "RULE_BASED_STOP_DECIDER"     type: "RuleBasedStopDecider" }
  task { name: "SPEED_BOUNDS_PRIORI_DECIDER" type: "\texttt{SpeedBoundsDecider}" }
  task { name: "SPEED_HEURISTIC_OPTIMIZER"   type: "\texttt{PathTimeHeuristicOptimizer}" }
  task { name: "SPEED_DECIDER"               type: "\texttt{SpeedDecider}" }
  task { name: "SPEED_BOUNDS_FINAL_DECIDER"  type: "\texttt{SpeedBoundsDecider}" }
  task { name: "PIECEWISE_JERK_SPEED"        type: "\texttt{PiecewiseJerkSpeedOptimizer}" }
}
\end{lstlisting}
...
\textbf{关键特征}\quad{}\texttt{PathDecider}仅处理静态障碍物，动态障碍物的处理交由速度规划阶段的ST图来完成。
...
\begin{figure}[htbp]
\centering
\adjustbox{max width=0.8\textwidth}{\input{../图片/fig_path_bounds_flow}}
...
第三，信息传递方向单向。路径阶段的输出即\texttt{PathData}传递给速度阶段作为已知条件，但路径阶段做出决策时的上下文信息，例如\emph{该路径依赖某动态障碍物在特定时刻移开}这一时间有效性前提，并未传递给速度阶段。速度阶段只知道路径在哪里，不知道路径为什么在那里。路径决策和速度规划之间因此缺乏时空约束的协调，这是第\ref{chap:st_corridor}章所分析的ST层问题的架构根源。第\ref{chap:st_corridor}章提出的时变通行带框架输出时间有效性约束，以ST上界的形式与\texttt{SpeedBoundsDecider}的输出在QP层面融合，补上路径-速度之间缺失的约束传递。
