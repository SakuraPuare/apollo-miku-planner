...
\textbf{本文在脉络中的定位}\quad{}上述工作整体上沿“先在路径层做静态建模、再在速度层补充时间信息”的思路推进了动态障碍物的处理。本文则进一步将到达时间函数引入路径边界构建阶段，把动态障碍物未来时刻的预测位置直接注入路径决策，使路径层对时间维度具备显式建模能力，与上述工作的速度层时序处理形成互补。
...
\textbf{动态安全距离与时变裕度}\quad{}\citet{zhaohongzhuan2025}提出基于横纵向椭圆动态安全距离模型的改进人工势场法，将速度与跟踪误差关联性引入横向虚拟安全空间；\citet{renhongzheng2025}基于车头时距与碰撞时间(TTC)定义变道危险区域和缓冲区域，为评估变道可行性提供量化依据；\citet{timevariantmargin2020}针对换道场景提出时变安全裕度的概念，将安全距离随时间动态调整，与本文差异化$\delta_i$的思想最为接近。
...
\textbf{本文在脉络中的定位}\quad{}上述工作整体上展示了威胁度评估与差异化安全距离在自动驾驶路径规划中的丰富建模思路。本文与之互补，将多因子威胁度评估与组级最优通行带算法相结合，使差异化$\delta_i$作为可插入参数无缝接入最大间隙公式，并通过引理与定理证明保证了差异化$\delta_i$不破坏最优性结构，为威胁度模型与全局决策的耦合提供了理论保证。
...
\item \textbf{群组感知时变通行带规划算法（GTOC）的设计与实现}\quad{}基于上述数学框架提出群组感知时变通行带规划算法（Group-aware Time-varying Optimal Corridor，GTOC）。GTOC通过引理证明最优分割的连续性、通过定理证明最大间隙策略的最优性，采用基于扫描线的$O(n \log n)$时变障碍物分组算法，在Apollo平台上实现并集成到PathBoundsDecider中。
...
本文共分为九章，沿“基础铺垫—统一框架—逐项创新—集成验证—总结展望”的横向叙事组织，各章内容安排如下。
...
第四章为GTOC总体框架与问题概览，速览多障碍物场景下的三个核心问题，建立时变最优通行带的统一数学模型，分析Apollo Baseline次优性的空间维度与时间维度根源，最后给出GTOC沿安全距离、决策粒度、时空协调三个维度的改进路线图。

第五章为多因子威胁度与差异化安全距离，针对安全裕度一刀切的源码缺陷，建立融合TTC、横向重叠率、相对速度、障碍物类型与交互密度的五因子威胁度评估模型，并将其映射为差异化安全裕度$\delta_i$。

第六章为组级最优通行带与最大间隙策略，针对独立决策矛盾导致的路径BLOCKED问题，提出基于扫描线的连通分量分组方法，证明最优分割连续性引理与最大间隙最优性定理，将组合搜索复杂度从$O(2^k)$降至$O(k\log k)$。

第七章为时空可通行走廊与路径-速度协调，针对动态障碍物被排除及路径-速度信息断层的双重缺陷，引入到达时间函数与预测轨迹构建时空可通行走廊，并将其边界与SpeedBoundsDecider的ST边界在速度QP层面双源融合。

第八章为系统集成与综合实验，给出GTOC集成至Apollo Planning流水线的工程方案与降级机制，通过四类共12个测试场景的仿真实验对比GTOC与Apollo Baseline的整体性能。

第九章为总结与展望，总结全文工作和贡献，讨论研究局限性和未来方向。
