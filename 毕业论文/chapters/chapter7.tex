...
本章对应MIKU的第三项改进，针对Apollo PathBoundsDecider通过\texttt{IsStatic()}过滤将动态障碍物完全排除在路径决策之外、以及路径-速度规划之间缺乏时空约束传递通道的双重缺陷，提出基于到达时间函数与预测轨迹的时空可通行走廊及其与SpeedBoundsDecider的双源约束融合机制。本章首先深入剖析动态障碍物排除在多障碍物场景下导致路径与速度规划相互脱节的源码根源与影响，进而引入基于二阶运动学模型的到达时间函数$\tau(s)$，将动态障碍物在自车到达时刻的预测位置纳入路径边界决策；将多障碍物统一规划的时间有效性条件投影至ST空间，构建时空可通行走廊；最后将走廊边界与SpeedBoundsDecider的ST边界在速度QP层面双源融合，在不修改速度规划模块结构的前提下建立路径-速度约束传递通道。本章的SL层输入来自第四章的差异化$\delta_i$与第五章的连通分量分组结果，输出最终接入第七章的Apollo系统集成方案。
...
在Apollo的路径-速度解耦架构中，如第二章2.3节所述，路径规划先于速度规划执行，速度规划在路径规划的结果上构建ST图。当动态障碍物不参与路径决策时，路径阶段可能生成一条直行路径，而速度阶段在该路径上发现动态障碍物横穿，不得不急刹或停车等待。如果路径阶段考虑了动态障碍物的预测轨迹，完全可以提前规划一条避让路径，在保持速度的同时安全通过。
...
需要指出的是，到达时间估计存在固有的循环依赖，$\tau(s)$用于计算时变通行带，通行带约束影响速度规划，而速度规划结果反过来决定了自车的实际到达时间。本文采用Apollo的周期性重规划机制，每100\,ms执行一次，隐式处理这一耦合，每个周期使用上一周期的速度输出更新$\tau(s)$，在连续多个规划周期中实现近似收敛。这种“单步延迟”策略在Apollo的路径-速度解耦框架中是标准做法，路径规划和速度规划本身就存在类似的耦合，其在城市道路工况下的实际效果已通过第七章的实验验证。
...
该max-min问题要求在所有$s$截面上同时选择一个固定的分割点$p$，使得最窄处的间隙最大。精确求解需要枚举$k+1$个$p$值，对每个$p$遍历所有离散$s$求最小间隙，复杂度为$O(kK)$，其中$K$为离散点数。算法\ref{alg:optimal_band}在实际实现中采用保守近似，以连通分量内所有障碍物均同时活跃的假设，在最密集的$s$截面上计算间隙并选取$p^{*}$。这一近似的安全性由\S\ref{sec:grouping_connected}的分析保证，最密集截面上的决策自然适用于更稀疏的截面。
...
Apollo的\texttt{PiecewiseJerkSpeedOptimizer}已在第二章2.3节给出完整描述，在等时间间隔$\Delta t=0.1$\,s上以纵向位置$s_j$、速度$\dot{s}_j$、加速度$\ddot{s}_j$为决策变量，目标函数惩罚位置/速度跟踪误差与加加速度，约束包含运动学等式约束、物理限制以及由\texttt{SpeedBoundsDecider}给出的ST边界
...
图\ref{fig:exp_ch7_st}对比ST平面。Baseline速度QP接收到行人ST禁行区，被迫从8\,m/s急刹至$\MOneBaselineMinV$\,m/s；MIKU因path阶段已经避开行人，速度QP无附加约束，全程保持8\,m/s匀速。MIKU子图中亦标出走廊节点$(\tau_k,s_k)$以体现显式传递通道，但因path已绕开，该节点不构成实际约束。
...
关键指标列于表\ref{tab:exp_ch7_metrics}。MIKU将平均巡航速度由\MOneBaselineAvgV\,m/s提升至\MOneMikuAvgV\,m/s，到达时间由\MOneBaselineTArrive\,s缩短至\MOneMikuTArrive\,s，纵向加减速峰值与jerk峰值均显著降低，体现时变投影对路径-速度协调断层的根本性修正。
...
实验结论与本章理论分析一致。当动态障碍物的预测轨迹可用且自车到达时刻晚于障碍物的SL占用窗口时，时变SL投影机制将Baseline下"行人挡路必须停车"的伪约束识别为"自车通过时行人已离开"的实际可通行情形，将整段YIELD替换为零干预匀速通过。该对照定量验证了式\eqref{eq:st_corridor}定义的时空走廊与式\eqref{eq:tighten_ub}速度QP上界收紧机制的工程价值。

\section{本章小结}

本章针对Apollo PathBoundsDecider将动态障碍物完全排除在路径决策之外、且路径-速度规划之间缺乏时空约束传递通道的双重缺陷，提出MIKU的第三项改进，即时空可通行走廊与路径-速度协调机制。在源码层面定位了\texttt{IsWithinPathDeciderScopeObstacle}函数中通过\texttt{IsStatic()}过滤排除动态障碍物的根源，结合Apollo团队在该函数中留下的\texttt{TODO}注释证实其为已知但未修复的设计缺陷；并通过横穿行人数值示例定性说明了排除动态障碍物后路径-速度协调断层的成因。在算法设计层面，引入基于二阶运动学模型的到达时间函数$\tau(s)$将动态障碍物的预测轨迹映射至自车到达时刻的SL投影，构建满足$t \geq \tau(s)$可达性约束的时空可通行走廊，并将走廊边界以ST上界的形式与SpeedBoundsDecider的输出在速度QP层面双源融合，建立path截断隐式通道与时间清单$\mathcal{T}$显式通道两条互补传递机制，在不修改速度规划模块结构的前提下打通此前缺失的路径-速度约束传递通道。本章的SL层输入来自第四章的差异化$\delta_i$和第五章的连通分量分组结果，输出最终接入第七章的Apollo系统集成方案。
