...
本章对应GTOC的第三项改进，针对Apollo PathBoundsDecider通过\texttt{IsStatic()}过滤将动态障碍物完全排除在路径决策之外、以及路径-速度规划之间缺乏时空约束传递通道的双重缺陷，提出基于到达时间函数与预测轨迹的时空可通行走廊及其与SpeedBoundsDecider的双源约束融合机制。本章首先深入剖析动态障碍物排除在多障碍物场景下导致路径与速度规划相互脱节的源码根源与影响，进而引入基于二阶运动学模型的到达时间函数$\tau(s)$，将动态障碍物在自车到达时刻的预测位置纳入路径边界决策；将时变最优通行带的时间有效性条件投影至ST空间，构建时空可通行走廊；最后将走廊边界与SpeedBoundsDecider的ST边界在速度QP层面双源融合，在不修改速度规划模块结构的前提下建立路径-速度约束传递通道。本章的SL层输入来自第五章的差异化$\delta_i$与第六章的连通分量分组结果，输出最终接入第八章的Apollo系统集成方案。
...
\begin{subfigure}[b]{0.32\textwidth}
  \centering
  \resizebox{\linewidth}{!}{\input{../图片/fig_dynamic_exclusion_a}}
  \caption{$t=0$ 行人未进入路径}
  \label{fig:dynamic_exclusion:a}
\end{subfigure}\hfill
\begin{subfigure}[b]{0.32\textwidth}
  \centering
  \resizebox{\linewidth}{!}{\input{../图片/fig_dynamic_exclusion_b}}
  \caption{$t=0.625$ s 行人半入路径}
  \label{fig:dynamic_exclusion:b}
\end{subfigure}\hfill
\begin{subfigure}[b]{0.32\textwidth}
  \centering
  \resizebox{\linewidth}{!}{\input{../图片/fig_dynamic_exclusion_c}}
...
因此，ST层面的问题不仅是“排除了动态障碍物”，更本质的问题是路径规划和速度规划之间缺乏时空约束的传递通道。改进方案需要建立这样一个通道，路径层面的时变决策能够以约束的形式传递给速度层面，确保两者的一致性。图\ref{fig:path_speed_breakage}示意Apollo解耦架构中Path阶段已掌握的时空决策依据如何被瓶颈截断，并预告本章后续将建立的两条传递通道。

\begin{figure}[htbp]
\centering
\resizebox{0.92\textwidth}{!}{\input{../图片/fig_path_speed_breakage}}
\caption{路径与速度规划之间的单向信息传递与时空约束缺口}
\caption*{\scriptsize 漏斗体现Apollo解耦框架的窄通道，仅$[l^-(s),l^+(s)]$得以通过。Path阶段掌握的$p^\star,\,g_{p^\star}(s),\,\tau(s)$与时间清单$\{(s_k,\tau_k)\}$被全部截断。下方两个绿色补丁通道是本章新增的传递机制，详见7.2.6节。}
\label{fig:path_speed_breakage}
\end{figure}

\section{ST层时空可通行走廊}
第四章建立的统一时变最优通行带数学框架与本章前节所识别的ST层问题，即动态障碍物被排除、路径-速度缺乏协调，要求将第五章和第六章的SL层理论扩展至时变场景。本节构建时空可通行走廊，利用Apollo Prediction模块提供的障碍物预测轨迹，将SL空间的最优通行带投影至ST空间，建立路径决策与速度规划之间的约束传递通道。
...
需要指出的是，到达时间估计存在固有的循环依赖，$\tau(s)$用于计算时变通行带，通行带约束影响速度规划，而速度规划结果反过来决定了自车的实际到达时间。本文采用Apollo的周期性重规划机制，每100\,ms执行一次，隐式处理这一耦合，每个周期使用上一周期的速度输出更新$\tau(s)$，在连续多个规划周期中实现近似收敛。这种“单步延迟”策略在Apollo的路径-速度解耦框架中是标准做法，路径规划和速度规划本身就存在类似的耦合，其在城市道路工况下的实际效果已通过第八章的实验验证。
...
此外，对于从Prediction模块接收到多条预测轨迹如车辆可能直行或变道的情况，当前实现取概率最高的轨迹。更精细的处理方式是对多条轨迹分别计算时变投影并取并集进行保守处理，但这会增加计算复杂度，留作后续工作。图\ref{fig:tvsl_pipeline}归纳了从Prediction模块原始轨迹到最终活跃障碍物集的四步数据流，到达时间$\tau(s)$在三个步骤中被共同引用；该步骤输出的活跃障碍物集即第六章定理1所需的输入。
...
基于7.2.1节的到达时间$\tau(s)$和7.2.2节的时变SL投影，本节完成第六章静态间隙优化向动态情形的推广，并把推广结果与$\tau(s)$合成ST平面上的二维可行域。本节交付两项定义。其一为作为$s$函数的时变间隙$g_p(s)$与全路径最优分割$p^*$，逐$s$截面给出空间维度的通行性判定。其二为时空可通行走廊$\Omega$，将SL通行性沿$\tau(s)$抬升至ST平面，作为7.2.4节速度规划约束的几何载体。
...
第六章定理1在每个$s$截面上仍然成立。这是因为定理1的证明过程即引理1的交换论证和定理1的直接代入验证仅依赖于$u$和$v$值的排序关系，不要求$u$和$v$是常数。只要在每个$s$处重新按$u_i(\tau(s))$排序并计算间隙，定理1的结论即可直接应用。
...
子图(a)给出"单$s$截面如何判定通行"的微观流程，选$s=s^\star=3.25$位于A内部，A的SL投影$[u_A,v_A]=[-1.2,0.3]$将道路切成两段间隙，由式\eqref{eq:time_varying_gap}得$g_0=0.8<W_{ego}$（左段不够装下自车）、$g_1=1.7\geq W_{ego}$（右段够），$g^\star=\max(g_0,g_1)=1.7$决定$p^\star=1$且该截面通行，自车恰嵌入右侧$g_1$间隙。
...
Apollo的\texttt{PiecewiseJerkSpeedOptimizer}已在第三章3.3节给出完整描述，在等时间间隔$\Delta t=0.1$\,s上以纵向位置$s_j$、速度$\dot{s}_j$、加速度$\ddot{s}_j$为决策变量，目标函数惩罚位置/速度跟踪误差与加加速度，约束包含运动学等式约束、物理限制以及由\texttt{SpeedBoundsDecider}给出的ST边界
...
在ST图中，式\eqref{eq:tighten_ub}的几何意义如图\ref{fig:st_constraint}所示，对应7.2.6节横穿行人示例的具体数值$(s_k=10\,\text{m},\,\tau_k=1.25\,\text{s})$。对于每个约束点，在时间轴$t < \tau_k$的区间内添加一条水平上界线$s \leq s_k$，禁止自车在该时间范围内超越位置$s_k$；速度曲线在约束点附近贴沿抵达后，行人随即让出，QP在平滑性惩罚下自然恢复至$v_{ref}$。

\begin{figure}[htbp]
\centering
\resizebox{0.5\textwidth}{!}{\input{../图片/fig_st_constraint}}
\caption{时间有效性约束在ST图中的几何表示}
\caption*{\scriptsize 红色区域为$\mathcal{T}$新增的禁止区，绿色粗线为与SpeedBoundsDecider上界取$\min$后的有效上界，深粉曲线为QP求解结果。约束点$(\tau_k,s_k)=(1.25,10)$对应7.2.6节横穿行人示例。}
...
\begin{figure}[htbp]
\centering
\resizebox{\textwidth}{!}{\input{../图片/fig_pathbounds_speedqp_dataflow}}
...
两个约束源的信息互补。SpeedBoundsDecider处理的是“路径上会遇到什么障碍物”，其ST边界基于给定路径的碰撞关系计算；ST走廊约束处理的是“为了让路径决策有效，速度需要满足什么条件”，其信息来源于路径阶段对动态障碍物预测轨迹的分析。这种融合直接解决了本章前节所识别的ST层问题，即路径决策信息未传递至速度规划。走廊约束正是此前缺失的路径-速度约束传递通道。
...
本章针对Apollo PathBoundsDecider将动态障碍物完全排除在路径决策之外、且路径-速度规划之间缺乏时空约束传递通道的双重缺陷，提出GTOC的第三项改进，即时空可通行走廊与路径-速度协调机制。在源码层面定位了\texttt{IsWithinPathDeciderScopeObstacle}函数中通过\texttt{IsStatic()}过滤排除动态障碍物的根源，结合Apollo团队在该函数中留下的\texttt{TODO}注释证实其为已知但未修复的设计缺陷；并通过横穿行人数值示例定性说明了排除动态障碍物后路径-速度协调断层的成因。在算法设计层面，引入基于二阶运动学模型的到达时间函数$\tau(s)$将动态障碍物的预测轨迹映射至自车到达时刻的SL投影，构建满足$t \geq \tau(s)$可达性约束的时空可通行走廊，并将走廊边界以ST上界的形式与SpeedBoundsDecider的输出在速度QP层面双源融合，建立path截断隐式通道与时间清单$\mathcal{T}$显式通道两条互补传递机制，在不修改速度规划模块结构的前提下打通此前缺失的路径-速度约束传递通道。本章的SL层输入来自第五章的差异化$\delta_i$和第六章的连通分量分组结果，输出最终接入第八章的Apollo系统集成方案。
