...
apollo::common::Status LaneFollowPath::Process(Frame* frame, ReferenceLineInfo* reference_line_info) {
    ...
    if (!AssessPath(&candidate_path_data, reference_line_info->mutable_path_data())) {
        ...
        BuildMikuSTDrivableBoundary(reference_line_info->path_data());
    }
...
bool LaneFollowPath::DecidePathBounds(std::vector<PathBoundary>* boundary) {
    ...
    if (config_.enable_miku_path_bounds()) {
        MikuPathBoundsConfig miku_config;
        miku_config.enable_dynamic_projection = config_.enable_miku_dynamic_projection();
        miku_config.enable_threat_buffer = config_.enable_miku_threat_buffer();
        miku_config.enable_grouping = config_.enable_miku_grouping();
        miku_config.enable_max_gap = config_.enable_miku_max_gap();
        miku_config.threat_buffer_min = config_.miku_threat_buffer_min();
        miku_config.threat_buffer_max = config_.miku_threat_buffer_max();
        miku_config.ttc_critical = config_.miku_ttc_critical();
        miku_config.ttc_max = config_.miku_ttc_max();
        miku_config.interaction_distance = config_.miku_interaction_distance();
        miku_config.velocity_limit = config_.miku_velocity_limit();
        if (!PathBoundsDeciderUtil::GetBoundaryFromMikuObstacles(
                    *reference_line_info_,
                    ...
                    &miku_speed_constraints_)) {
            AERROR << "Failed to apply MIKU obstacle path boundary.";
            return false;
        }
        path_bound.set_label(absl::StrCat("miku/", "self"));
    } else {
...
void LaneFollowPath::BuildMikuSTDrivableBoundary(const PathData& path_data) {
    if (!config_.enable_miku_path_bounds() || miku_speed_constraints_.empty() || path_data.Empty()) {
        return;
    }
    std::vector<std::pair<double, double>> path_constraints;
    for (const auto& constraint : miku_speed_constraints_) {
        common::PathPoint path_point;
        if (!path_data.GetPathPointWithRefS(constraint.first, &path_point)) {
            continue;
        }
        if (constraint.second <= 0.0) {
            continue;
        }
        path_constraints.emplace_back(path_point.s(), constraint.second);
    }
    if (path_constraints.empty()) {
        return;
    }

    const double total_time = FLAGS_trajectory_time_length;
    const double delta_t = std::max(FLAGS_trajectory_time_resolution, 1.0e-3);
    const double path_length = path_data.discretized_path().Length();
    std::vector<std::tuple<double, double, double>> st_boundaries;
    std::vector<std::tuple<double, double, double>> vt_boundaries;
    for (double curr_t = 0.0; curr_t <= total_time + 1.0e-6; curr_t += delta_t) {
        double s_upper = path_length;
        for (const auto& constraint : path_constraints) {
            if (curr_t < constraint.second - 1.0e-6) {
                s_upper = std::min(s_upper, constraint.first);
            }
        }
        st_boundaries.emplace_back(curr_t, 0.0, std::max(0.0, s_upper));
        vt_boundaries.emplace_back(curr_t, std::numeric_limits<double>::lowest(), std::numeric_limits<double>::max());
    }

    reference_line_info_->mutable_st_graph_data()->SetSTDrivableBoundary(st_boundaries, vt_boundaries);
    AINFO << "MIKU ST corridor constraints: " << path_constraints.size();
}
