...
Status PiecewiseJerkSpeedOptimizer::Process(
    ...
    double delta_t = std::max(FLAGS_trajectory_time_resolution, 1.0e-3);
    ...
    const auto& st_drivable_boundary = st_graph_data.st_drivable_boundary();
    const bool has_st_drivable_boundary = st_drivable_boundary.st_boundary_size() > 0;
    ...
    for (int i = 0; i < num_of_knots; ++i) {
        ...
        if (has_st_drivable_boundary) {
            const int boundary_index = std::min(i, st_drivable_boundary.st_boundary_size() - 1);
            const auto& drivable_bound = st_drivable_boundary.st_boundary(boundary_index);
            s_lower_bound = std::fmax(s_lower_bound, drivable_bound.s_lower());
            s_upper_bound = std::fmin(s_upper_bound, drivable_bound.s_upper());
            print_debug.AddPoint("miku_st_corridor_upper", curr_t, drivable_bound.s_upper());
            print_debug.AddPoint("miku_st_corridor_lower", curr_t, drivable_bound.s_lower());
        }
