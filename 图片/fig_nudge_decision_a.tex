% RIGHT_NUDGE 决策对边界影响（子图 a）
% 障碍物在参考线上方，右绕 → 上界被压低至 u_i = l_i^min - δ
\begin{tikzpicture}[scale=1.0, transform shape]
  ...
  % 障碍物 O_i（参考线上方）
  \fill[dangerred!70, draw=dangerred, thick] (2.5,0.5) rectangle (4.5,1.5);
  \node[white, font=\footnotesize\bfseries] at (3.5,1.0) {$O_i$};

  % 障碍物膨胀虚线框
  \draw[dangerred!70, dashed, thick] (2.2,0.1) rectangle (4.8,1.9);
  \node[dangerred!70, font=\scriptsize, right] at (4.9,1.5) {$+\delta$};

  % 上界 l+(s)：从道路上界 → 压低到 u_i → 恢复道路上界
  % 与道路边界连起来
  \draw[deeppink, very thick]
    (0, 2.5) -- (2.2, 2.5) -- (2.2, 0.1) -- (4.8, 0.1) -- (4.8, 2.5) -- (7.5, 2.5);
  \node[deeppink, font=\small] at (3.5,-0.3) {$l^+(s) = u_i$};

  % 下界 l-(s)：保持道路下界不变
  \draw[deepblue, very thick, dashed]
    (0,-2.5) -- (7.5,-2.5);
  \node[deepblue, font=\small, above left] at (7.5,-2.3) {$l^-(s)$};

  % u_i 标注
  \draw[{Stealth}-{Stealth}, thin, deeppink] (5.2, 0.1) -- (5.2, 2.5);
  \node[deeppink, font=\scriptsize, right] at (5.3, 1.3) {被压缩};

  % 自车轨迹：起点终点在 l=0，向下远离障碍物弯曲
  \draw[deepblue, very thick, -{Stealth}]
    plot[smooth, tension=0.55] coordinates
      {(0.5, 0) (1.5, -0.3) (2.5, -0.8) (3.5, -1.0) (4.5, -0.8) (5.5, -0.3) (6.5, 0)};
  \node[deepblue, font=\footnotesize\bfseries, below] at (3.5,-1.2) {自车轨迹};
