...
\node[stepimp] (s6) at (0,-3.0)  {\textbf{Step 6} 合并全局 path\_boundary};
\node[stepimp] (s7) at (0,-4.7)  {\textbf{Step 7} ST 时空可通行走廊 $\Omega$};

% 输出（左侧）与跨接口的速度 QP
\node[outnode] (o1) at (-5.8,-3.0) {path\_boundary};
\node[outnode] (o2) at (-5.8,-4.7) {st\_corridor};
...
% 走廊注入速度 QP（跨解耦接口）
\draw[injarr] (o2.south) -- node[right, font=\footnotesize, deeppink] {\texttt{STDrivableBoundary} 注入上界} (qp.north);

% 复杂度标注（右侧）
\node[font=\footnotesize, deeppink, anchor=west] at (4.4, 5.5)  {$O(n N_s)$};
\node[font=\footnotesize, deeppink, anchor=west] at (4.4, 3.8)  {$O(n)$};
\node[font=\footnotesize, deeppink, anchor=west] at (4.4, 2.1)  {$O(n)$};
\node[font=\footnotesize, deeppink, anchor=west] at (4.4, 0.4)  {$O(n\log n)$};
\node[font=\footnotesize, deeppink, anchor=west] at (4.4,-1.3)  {$O(n)$};
\node[font=\footnotesize, deeppink, anchor=west] at (4.4,-3.0)  {$O(m)$};
\node[font=\footnotesize, deeppink, anchor=west] at (4.4,-4.7)  {$O(n N_s)$};
