% 场景03 速度/加速度 Baseline vs GTOC 对比
...
\begin{axis}[
    ...
    xmin=0, xmax=7.0, ymin=0, ymax=10.4,
...
% v_ref 参考线
\draw[black!50, dashed, thick] (axis cs:0,8.00) -- (axis cs:7.00,8.00);
\node[black!50, font=\tiny, anchor=south east] at (axis cs:7.00,8.00) {$v_{ref}$};
\addplot[deepblue, line width=2pt]
    table[col sep=comma, x=t, y=v_qp, discard if not={mode}{baseline}] {\datapath st_curves.csv};
\addlegendentry{Baseline\,(\,$\bar{v}{=}5.44$,\,$|a|_{max}{=}2.45$,\,$|j|_{max}{=}6.37$,\,$s_{end}{=}39.0$\,m\,)}
\addplot[vibrantgreen, line width=2pt]
    table[col sep=comma, x=t, y=v_qp, discard if not={mode}{gtoc}] {\datapath st_curves.csv};
\addlegendentry{GTOC\,(\,$\bar{v}{=}8.00$,\,$|a|_{max}{=}0.00$,\,$|j|_{max}{=}0.00$,\,$s_{end}{=}56.0$\,m\,)}
\end{axis}
\end{tikzpicture}
\caption{速度对比}
\end{subfigure}
