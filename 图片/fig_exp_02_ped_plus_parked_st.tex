...
% 场景02 ST 平面 Baseline vs MIKU 对比
...
\begin{axis}[
    ...
    xmin=0, xmax=6.5, ymin=0, ymax=48.0,
    restrict y to domain*=0:48.0,
...
\addplot[name path=stlo_baseline_1, draw=none, forget plot]
    table[col sep=comma, x=t, y=s_lo, discard if not2={mode}{baseline}{obs_name}{停车}] {\datapath st_bounds.csv};
\addplot[name path=sthi_baseline_1, draw=none, forget plot]
    table[col sep=comma, x=t, y=s_hi, discard if not2={mode}{baseline}{obs_name}{停车}] {\datapath st_bounds.csv};
...
\addplot[pattern=north east lines, pattern color=deeppink!60, draw=deeppink!60, semithick, forget plot]
    coordinates {(0,7.7500) (0.9688,7.7500) (0.9688,48.0) (0,48.0)} \closedcycle;
\node[deeppink, font=\tiny, fill=white, fill opacity=0.85, text opacity=1, inner sep=1pt] at (axis cs:1.369,10.250) {$\tau_k{=}0.97,\,s_k{=}7.8$};
% MIKU 走廊关键点（tau_k 标注）
