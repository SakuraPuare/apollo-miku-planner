...
% 场景06 速度/加速度 Baseline vs MIKU 对比
...
\addlegendentry{Baseline\,(\,$\bar{v}{=}4.80$,\,$|a|_{max}{=}2.82$,\,$|j|_{max}{=}9.33$,\,$s_{end}{=}31.0$\,m\,)}
\addplot[vibrantgreen, line width=2pt]
    table[col sep=comma, x=t, y=v_qp, discard if not={mode}{miku}] {\datapath st_curves.csv};
\addlegendentry{MIKU\,(\,$\bar{v}{=}4.76$,\,$|a|_{max}{=}1.32$,\,$|j|_{max}{=}1.25$,\,$s_{end}{=}31.0$\,m\,)}
\end{axis}
\end{tikzpicture}
\caption{速度对比}
\end{subfigure}
