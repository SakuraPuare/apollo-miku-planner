...
若真实障碍物占据满足 $O_i^{\rm true}(t)\subseteq O_i^{\rm rob}(t)$，执行适配器在相邻输出节点间按速度 QP 的恒加速度纵向模型滚放、横向中心作线性插值，且该轨迹始终位于按式\eqref{eq:robust-tube}构造的时空走廊内，则执行轨迹与 $O_i^{\rm true}(t)$ 不相交。实现对纵向二次重叠根和横向线性重叠区间作解析检查；任意模型失配或控制跟踪误差仍不在证书范围内。
\end{proposition}
\begin{proof}
