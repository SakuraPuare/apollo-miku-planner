...
\end{align}
式\eqref{eq:pass}要求自车在障碍进入冲突区前完成通过，式\eqref{eq:yield}要求自车在障碍离开冲突区前保持在其后方。中间安全窗同时施加前一占用的后通过上界与后一占用的先通过下界。两者共同构成双向时序走廊，避免把所有动态冲突都退化为停车让行。为使联合搜索具有可验证的剪枝，令空间走廊在路径站点 $s_j$ 的横向盒为 $[l_j^-,l_j^+]$，并定义
\begin{equation}
 d_j=\begin{cases}0,&l_j^-\leq0\leq l_j^+,\\
 \min(|l_j^-|,|l_j^+|),&\text{otherwise.}\end{cases}
 \qquad L_h=w_l\frac{1}{N}\sum_{j=1}^N d_j^2 .
 \label{eq:corridor-lb}
\end{equation}
固定标签目标 $J$ 的横向项为 $w_lN^{-1}\sum_j l_{path}(s_j)^2$，且 $l_{path}(s_j)\in[l_j^-,l_j^+]$，其余终点误差、加速度和 jerk 项均非负，故 $J\geq L_h$。同一空间走廊下的时间标签共享该下界，惰性搜索可在 incumbent 不小于所有未展开下界时安全剪枝并返回 $U-L$；若下界不足以剪枝，则仍完整评估有限域。连续扫掠矩形验证失败的候选被 fail-closed，不能以“QP solved”替代安全可行性。

\begin{proposition}[走廊下界的可采纳性]
若固定标签的横向路径满足 $l_{path}(s_j)\in[l_j^-,l_j^+]$，且目标中的横向项权重 $w_l\geq0$，则由式\eqref{eq:corridor-lb}定义的 $L_h$ 不超过该标签的完整目标 $J$。因此，以 $L_h$ 作为空间分支及其时间子标签的下界不会剪去有限域中的最优可行标签。
\end{proposition}
\begin{proof}
区间 $[l_j^-,l_j^+]$ 中到零点的最小绝对距离为 $d_j$，故 $l_{path}(s_j)^2\geq d_j^2$。乘以非负权重并对站点求平均得到横向项不小于 $L_h$；目标中的终点误差、加速度和 jerk 项均非负，故 $J\geq L_h$。同一空间分支下时间标签不改变横向区间，因此共享该下界。惰性搜索仅在下界不小于 incumbent 时剪枝，结论随即成立。
\end{proof}
