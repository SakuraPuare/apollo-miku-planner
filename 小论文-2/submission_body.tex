...
若真实障碍物占据满足 $O_i^{\rm true}(t)\subseteq O_i^{\rm rob}(t)$，执行适配器在相邻输出采样间对双方中心作分段线性插值，且该插值轨迹始终位于按式\eqref{eq:robust-tube}构造的时空走廊内，则执行轨迹与 $O_i^{\rm true}(t)$ 不相交。本命题不覆盖 QP 节点间的二次恒加速度轨迹、模型失配或控制跟踪误差。
\end{proposition}
\begin{proof}
