...
\end{align}
式\eqref{eq:pass}要求自车在障碍进入冲突区前完成通过，式\eqref{eq:yield}要求自车在障碍离开冲突区前保持在其后方。中间安全窗同时施加前一占用的后通过上界与后一占用的先通过下界。两者共同构成双向时序走廊，避免把所有动态冲突都退化为停车让行。候选的下界取为零，固定标签的目标由终点误差、加速度、jerk 和横向偏移构成，均为非负项。惰性搜索在有限域耗尽或 incumbent 不大于所有未展开下界时返回 $U-L=0$；若设置评估预算，则仅报告可审计的 $U-L$ 间隙。连续扫掠矩形验证失败的候选被 fail-closed，不能以“QP solved”替代安全可行性。
