...
残差构造随时间增长的保守位置包络；在中心点线性插值假设下包络覆盖样本间
中心轨迹；同时对每个发布状态的矩形四角进行投影，形成被 ST 约束和连续证书消费的保守时变占用包络。规则元素被记录，但当前 Frenet planner 不主动优化交通控制规则。

进一步对四个固定 Lankershim 文件执行批量 adapter smoke，四个场景均成功建立
起终点可达链并调用两种方法。受限转换后的诊断结果为：B0/MIKU 成功率为
25%/0%，碰撞率为 75%/50%，MIKU 的中位耗时为 126.4 ms（最大 324.5 ms）。
...
该结果证明的是“公开 XML -> 官方路线/逐状态形状姿态包络 -> planner 调用”链路可执行，
不是对未覆盖交通规则策略、感知误差或外部基线的忠实
评测。因此论文仍不宣称 CommonRoad benchmark 性能；二区送审前仍需完整
规则合规策略、公开场景批量指标和一般 leaderboard 外部性能对比。
