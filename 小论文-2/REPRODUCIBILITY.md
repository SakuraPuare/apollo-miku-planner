...
uv run --no-project --python 3.11 --with commonroad-reactive-planner python 可视化/run_commonroad_reactive.py --steps 40
uv run --no-project --python 3.11 --with commonroad-reactive-planner python 可视化/run_commonroad_miku_native.py --steps 40
uv run python 可视化/generate_commonroad_macros.py
...
| CommonRoad Reactive Planner 正式覆盖 | 4 个公开 Lankershim XML | `run_commonroad_reactive.py --steps 40`（隔离 3.11 runtime） | `commonroad_reactive_results.json` 与 `commonroad_reactive_solutions/*.xml`；官方 reader、路线参考线、Reactive Planner、标准 solution writer 和 drivability checker；2 个 valid solution、2 个 planner failure，失败计入结果 |
| CommonRoad MIKU 原生输出边界 | 4 个公开 Lankershim XML × B0/MIKU | `run_commonroad_miku_native.py --steps 40`（隔离 3.12 runtime） | `commonroad_miku_native_results.json` 与 `commonroad_miku_native_solutions/*.xml`；官方 planning problem、RoutePlanner reference path、逐状态矩形形状/姿态占用包络、KSState、solution writer 和 evaluator 均调用；全部动态障碍物进入输入（24--42 个/场景），MIKU 4 次 planner failure、0 个 valid solution；交通控制规则仅记录，不作一般 leaderboard 分数 |
