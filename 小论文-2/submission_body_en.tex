...
If $O_i^{\rm true}(t)\subseteq O_i^{\rm rob}(t)$, the execution adapter rolls out the speed-QP longitudinal state with constant acceleration between knots and linearly interpolates the lateral center, and that rollout remains in the corridor excluding $O_i^{\rm rob}$ and the vehicle footprint, it cannot intersect $O_i^{\rm true}(t)$. The implementation checks the resulting quadratic longitudinal overlap roots and linear lateral overlap intervals. Arbitrary model mismatch or tracking error remains outside the certificate.
\end{proposition}
\begin{proof}
