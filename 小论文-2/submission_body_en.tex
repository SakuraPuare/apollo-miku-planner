...
If $O_i^{\rm true}(t)\subseteq O_i^{\rm rob}(t)$, the execution adapter linearly interpolates both centers between output samples, and that interpolant remains in the corridor excluding $O_i^{\rm rob}$ and the vehicle footprint, it cannot intersect $O_i^{\rm true}(t)$. This proposition does not cover the quadratic constant-acceleration curve between QP knots, model mismatch, or tracking error.
\end{proposition}
\begin{proof}
