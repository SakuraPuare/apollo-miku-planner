...
\caption{Teaser: in a deterministic delayed-crossing case, the time-blind B0 baseline stops while MIKU retains a \texttt{pass-before} spatiotemporal label and reaches the goal. The left panel shows the $S$--$L$ trajectories; the right panel shows the $S$--$T$ trajectories and obstacle occupancy. The figure is generated by replaying the fixed experiment case and is a mechanism diagnostic, not a real-vehicle deployment result.}
\label{fig:teaser}
\end{figure*}
...
\end{align}
Here $s_T=\min(s_N^{\rm ub},s_{\rm goal})$ is the terminal reference used by the speed QP. No separate surrogate objective is used by the certificate.
To obtain an admissible branch bound without weakening the certificate, let the spatial corridor at station $s_j$ be $[l_j^-,l_j^+]$ and define
\begin{equation}
\begin{aligned}
d_j&=\begin{cases}0,&l_j^-\leq0\leq l_j^+,\\
\min(|l_j^-|,|l_j^+|),&\text{otherwise,}\end{cases}
\\
L_h&=w_l\sum_{j=1}^{N}d_j^2 .
\end{aligned}
\label{eq:corridor-lb}
\end{equation}
The geometric bound $L_h$ is a valid but deliberately loose bound on the lateral term. In the implementation, each spatial branch is tightened to the solved path-QP value $L_{\rm path}=\min J_l$ for that branch; all speed-QP terms are non-negative, hence $L_{\rm path}\leq J_{\rm QP}$ for every temporal child. Temporal labels under one spatial corridor share this bound, so lazy search may prune an open item when its bound is no smaller than the incumbent while preserving the finite-domain $U-L$ certificate. If the bound is insufficient, the finite domain is still exhaustively evaluated.

\begin{proposition}[Admissibility of the branch lower bound]
Let $L_{\rm path}$ be the optimum of the path QP for a fixed spatial branch. Since every speed-QP term in \eqref{eq:qp-objective} is non-negative, $L_{\rm path}$ does not exceed the complete objective $J_{\rm QP}$ of any feasible temporal child. Using $L_{\rm path}$ for a spatial branch therefore cannot prune the finite-domain optimal feasible label.
\end{proposition}
\begin{proof}
The path-QP solution minimizes $J_l$ over the fixed spatial corridor. A temporal child reuses that spatial path and adds the non-negative speed cost $J_s$, so $J_{\rm QP}=J_l+J_s\geq L_{\rm path}$. Pruning only occurs when this bound is no smaller than the incumbent, which preserves the finite-domain optimum.
\end{proof}
...
\caption{Finite-domain stress diagnostic with two feasible bands per separated spatial layer and one dynamic crossing. The solved path-QP value supplies an admissible branch lower bound; the domain/evaluation gap exposes actual pruning.}
\label{fig:joint-stress}
\end{figure}

Increasing the separated spatial-conflict layers from one to \StressMaxLayers{} produces \StressSpatialCount{} spatial homotopies and \StressDomainSize{} joint labels in the largest instance (Fig.~\ref{fig:joint-stress}). The search performs only \StressEvaluatedCount{} QP validations and takes a median \StressRuntimeMedian\,ms over three repeats. This verifies multi-branch enumeration, an admissible path-QP branch bound, and actual pruning while retaining an explicit exponential worst-case caveat.

\subsection{External CommonRoad Audit}
\label{sec:commonroad}

To separate public-benchmark provenance from the paired synthetic protocol, we
also pin four NGSIM Lankershim scenarios from CommonRoad~\cite{althoff2017commonroad}.
The official CommonRoad reader, route/reference-path utilities, Reactive Planner,
solution writer, and Drivability Checker~\cite{pek2020drivability} are executed
in an isolated environment with $\Delta t=0.1$ s and a 40-step horizon. The
official competitor runner completes all four scenario files: two trajectories
are valid under the official dynamics, collision, boundary, and goal checks,
and two runs terminate as planner failures (Table~\ref{tab:commonroad}).

The MIKU-side result is reported in two layers. The native boundary runner
reads the official planning problem, converts any successful rollout to
standard \texttt{KSState}/\texttt{PlanningProblemSolution} objects, invokes the
official evaluator, and retains planner failures. On these four files MIKU
produces four explicit planner failures and zero valid solutions; this is a
negative result, not an external superiority claim. Its planner input uses the
official reference route and every published dynamic-obstacle rectangle and
pose. The four body corners at each state are projected into Frenet coordinates
and interpolated as a conservative time-varying occupancy envelope consumed by
the ST mapper and continuous safety certificate. Lanelet routing and the
planning-problem goal remain official; traffic-control rules are recorded but
are not optimized by the current Frenet planner. The resulting benchmark is
therefore scoped and auditable, not a general leaderboard claim.

\begin{table}[t]
\caption{Pinned CommonRoad external audit. The native-boundary row uses official
lanelet routing, sampled shape/pose occupancy envelopes, solution writing, and
evaluation; it is not a general leaderboard score.}
\label{tab:commonroad}
\centering
\scriptsize
\begin{adjustbox}{max width=\columnwidth}
\begin{tabular}{lrrrr}
\toprule
Protocol & $n$ & Valid/Success(\%) & Collision(\%) & Median ms\\
\midrule
Official Reactive Planner & \CRReactiveScenarioCount & \CRReactiveValidCount/\CRReactiveValidPct & -- & --\\
Native MIKU boundary & \CRNativeMikuScenarioCount & \CRNativeMikuValidCount/\CRNativeMikuValidPct & -- & \CRNativeMikuMedianRuntime\\
Adapter B0 & \CRAdapterCaseCount & \CRBZeroSuccessPct & \CRBZeroCollisionPct & \CRBZeroMedianRuntime\\
Adapter MIKU & \CRAdapterCaseCount & \CRMikuSuccessPct & \CRMikuCollisionPct & \CRMikuMedianRuntime\\
\bottomrule
\end{tabular}
\end{adjustbox}
\end{table}
...
For system provenance, the repository pins Apollo Planning commit
\texttt{57460908954e3188f640a813d26180e862d62a5f}, indexes the Planning/CyberRT/Dreamview
fixtures, and records the available runtime artifacts. Those manifests establish
source and asset lineage; no new native Apollo run was launched during this
manuscript rebuild, so the metrics above remain algorithm-level results rather
than native Apollo runtime claims.
