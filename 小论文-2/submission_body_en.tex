...
\caption{Finite-domain stress diagnostic with two feasible bands per separated spatial layer and one dynamic crossing. Domain size equals QP evaluations because the present zero lower bound gives no pruning.}
\label{fig:joint-stress}
\end{figure}

Increasing the separated spatial-conflict layers from one to \StressMaxLayers{} produces \StressSpatialCount{} spatial homotopies and \StressDomainSize{} joint labels in the largest instance (Fig.~\ref{fig:joint-stress}). The search performs \StressEvaluatedCount{} QP validations and takes a median \StressRuntimeMedian\,ms over three repeats. This verifies multi-branch enumeration and its certificate while quantifying, rather than concealing, the exponential worst case under the current zero bound.
