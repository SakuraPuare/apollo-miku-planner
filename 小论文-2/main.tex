...
\small
\noindent
路径--速度解耦规划器在路径阶段处理动态多障碍物时，时间无关投影会合并不同时刻的占据，逐障碍物避让又可能因局部选向冲突过度收缩横向可行域。本文提出多障碍物交互感知运动学约束统一方法（Multi-Obstacle Interaction-Aware Kinematic Constraint Unification, MIKU）。MIKU 先将道路与障碍物物理边界统一转化为自车中心约束，再以扫描线形成纵向连通分量，将组内避让组合化为排序后的连续分割与最大间隙选择，并用当前周期到达时间初值 $\tau(s)$ 查询动态占据。单截面组内求解由 $O(2^k)$ 组合枚举化为 $O(k\log k)$ 排序与扫描；固定种子随机反例检查将扫描解与 $2^k$ 穷举解逐例对照。在 Apollo 路径--速度解耦规划环境的 8 个确定性场景中，4 个目标压力场景的基线与 MIKU 终点到达数分别为 0/4 和 4/4，4 个配对场景中两者均为 4/4。关闭速度侧走廊注入后所有已报告指标不变，因而本文将其降级为可选接口，不将其作为已获得独立实验支持的性能贡献。这些结果支持 MIKU 在所考察构型中改善边界保守与局部选向冲突，不作统计普适性外推。
...
本文提出多障碍物交互感知运动学约束统一方法（Multi-Obstacle Interaction-Aware Kinematic Constraint Unification, MIKU）。其主要贡献收敛为两项：第一，统一道路中心可行边界与障碍物中心禁行边界，将多障碍物避让转化为连通分量上的最大间隙问题，证明最优分割在排序中连续，并把单组搜索由 $O(2^k)$ 降至 $O(k\log k)$；第二，以当前周期的到达时间初值 $\tau(s)$ 查询动态占据，构造时变横向通行带，并在 Apollo 解耦求解链中保持两个 QP 的内部结构不变。多因子差异化裕度作为边界参数化模块；速度侧走廊上界保留为可选接口，不列为已被当前实验独立验证的贡献。
...
\begin{equation}
    g_p(s)=U_p(s)-V_p(s),\qquad p=0,1,\ldots,k,
...
\begin{align*}
 V_0(s)&=l_{road}^{-}(s),\\
 V_p(s)&=\max\!\left(l_{road}^{-}(s),\max_{1\leq q\leq p}v_{(q)}(\tau(s))\right),\\
 U_p(s)&=\min\!\left(l_{road}^{+}(s),u_{(p+1)}(\tau(s))\right)\quad(p<k),\\
 U_k(s)&=l_{road}^{+}(s).
\end{align*}
引理\ref{lem:continuous_partition}与定理\ref{thm:max_gap}给出逐截面最优；将其拼接为一条连续偏移曲线时，还需在同一连通分量内固定组级分割。这里固定的是各障碍物到 $\mathcal{L}$、$\mathcal{R}$ 的指派，即全程保持同侧绕行，而非固定排序序号 $p$。若分量覆盖区间内各 $u_i(\tau(s))$ 的相对次序不随 $s$ 翻转，则序号分割与指派分割一致；发生翻转时，采用与链式分量相同的保守包络。指派固定后，最优分割取使整段最薄截面最宽的极大极小解
\begin{equation}
...
约束传播包含隐式与显式两条通道。若某 $s_k$ 处 $g_{p^{*}}(s_k)<\epsilon$，路径有效区间在该真实不可行位置终止，禁行切片通过路径几何长度隐式传达；其余由到达时序产生的条件则通过式\eqref{eq:tighten_ub}显式写入速度上界。两条通道分别传递空间不可行位置和时间开放条件，且都不改变路径 QP 与速度 QP 的内部求解形式。
...
设速度 QP 的目标为连续凸二次函数，决策变量受有界的速度、加速度和位置盒约束。若存在分割 $p^{*}$ 使 $\min_s g_{p^{*}}(s)\geq\epsilon$，且存在一条离散运动学轨迹 $\hat{\mathbf{x}}$ 同时满足原下界、式\eqref{eq:tighten_ub}收紧后的上界及全部运动学等式，则走廊注入后的可行域非空，速度 QP 存在最优解。特别地，当静止等待轨迹满足原下界与物理约束，且所有走廊上界均不低于当前纵向位置时，上述可行性条件成立。
\end{proposition}
...
\For{每个路径离散位置 $s$}{
  ...
  \For{每个连通分量 $C$}{
    ...
    用道路中心边界裁剪前缀最大值，按式\eqref{eq:time_varying_gap}计算 $k+1$ 个间隙\;
...
本文提出 MIKU 多障碍物交互感知运动学约束统一方法，通过统一的自车中心边界、时变占据投影与组级最大间隙协调路径--速度解耦规划中的多障碍物边界构造。理论部分证明了最优分割的连续性与前缀最大间隙解的精确最优性，并将单截面组合搜索由 $O(2^k)$ 降至 $O(k\log k)$；固定种子的自动穷举对照未发现反例。在 4 个目标压力场景中，MIKU 将终点到达数由 0/4 提升至 4/4；在 4 个可比场景中保持 4/4 到达数和同量级求解时间。当前消融支持时变投影、组级间隙决策和差异化裕度在对应目标构型中的作用；关闭速度侧走廊注入后已报告指标不变，故其仅保留为可选接口。当前结论限于这些确定性构型；随机闭环场景、安全指标、公平强基线与时间窗约束的独立效果仍需完成后才能扩展性能结论。
