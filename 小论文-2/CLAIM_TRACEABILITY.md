...
| 有限联合同伦搜索与最优性 gap | 新算法接口 | `joint_homotopy_search.py::bounded_lazy_joint_search`, `experiment_methods.run_method` | 100 个随机域穷举、预算截断、下界反例；`joint_search_stress_*` 的 2--40 联合候选规模曲线；主/滚动原始行的 `joint_*` 字段 | 证书只相对于显式有限候选域；不宣称连续全局最优；零下界导致压力协议全枚举，主实验多数域仍只有 1 个候选 |
