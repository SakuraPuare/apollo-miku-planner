...
| 鲁棒占据传播有界预测误差 | 式(8)、鲁棒安全命题 | `Obstacle.uncertainty_*`, `_miku_path_bounds`, `st_boundary_mapper`, continuous certifier | 路径/ST 投影、分段线性扫掠和候选 fail-closed 测试 | 仅当真值误差落入声明的有界管、且插值/速度界成立时 |
...
| 主实验碰撞率下降 | 摘要、实验4.2 | 真值几何评价 | MIKU/B0 `0.14%/6.0%`, 差 `-5.9 pp`, CI `[-7.6,-4.1]`, `p=9.09e-13` | 同上；总体差异不代表每个场景族均改善 |
| 关键模块具有独立贡献 | 实验4.4 | `run_randomized_ablation.py` | A2--A7 的配对 bootstrap 与 McNemar | A1 仅作初始化，不声称独立显著收益 |
| 滚动规划延续单周期收益 | 实验4.5 | `closed_loop.py`, rolling runner | 700 对：到达率差 `+9.4 pp`, CI `[6.9,12.1]`, `p=1.31e-11`; 碰撞差 `-0.57 pp`, `p=.344` | 不等同于实车全系统闭环 |
