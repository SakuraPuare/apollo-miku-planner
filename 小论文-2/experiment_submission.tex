...
\caption{非平凡有限联合域的规模诊断。每层含两个可行空间带，并叠加一个动态横穿。候选域与 QP 评估数重合，说明当前零下界不产生剪枝。}
\label{fig:joint-stress}
\end{figure}

图~\ref{fig:joint-stress} 将相互分离的空间冲突层从 1 增至 \StressMaxLayers。最大实例含 \StressSpatialCount 个空间同伦与 \StressDomainSize 个联合候选，搜索执行 \StressEvaluatedCount 次 QP 验收，三次重复的中位完整耗时为 \StressRuntimeMedian\,ms。该压力协议证明实现能枚举并证书化多分支域，也同时量化了零下界下的指数最坏代价，不作剪枝优势主张。
