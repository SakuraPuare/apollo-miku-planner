...
安全性方面，MIKU 碰撞率为 \RandAllMikuCollisionPct\%，B0 为 \RandAllBZeroCollisionPct\%，配对差为 \RandAllMikuVsBZeroCollisionDiff 个百分点，95\% CI [\RandAllMikuVsBZeroCollisionCiLow,\RandAllMikuVsBZeroCollisionCiHigh]，McNemar $p=\RandAllMikuVsBZeroCollisionMcNemarP$。总体通行率提升没有以增加碰撞为代价；但预测含噪子集以更高降级停止换取零碰撞，不能概括为单向收益。MIKU 完整周期 P95 为 \RandAllMikuRuntimePninetyfive\,ms。

\begin{figure}[htbp]
\centering
\includegraphics[width=0.98\textwidth]{generated/evidence_dashboard.pdf}
\caption{由原始 CSV 生成的配对证据总览：(a) MIKU--B0 分场景到达率差及 95\% CI；(b) 完整规划耗时 ECDF；(c) 完整 MIKU 相对各消融项的到达率差；(d) MIKU 分场景成功、安全未到达与碰撞构成。}
...
如图~\ref{fig:evidence-dashboard}(a,d) 所示，MIKU 在窄路多障碍中提升 \RandNarrowMikuVsBZeroSuccessDiff 个百分点，在延迟横穿中提升 \RandDelayedMikuVsBZeroSuccessDiff 个百分点。预测含噪场景到达率变化为 \RandNoiseMikuVsBZeroSuccessDiff 个百分点（下降），碰撞率配对差为 \RandNoiseMikuVsBZeroCollisionDiff 个百分点（下降），并伴随高降级停止率，体现安全--可达性权衡。图~\ref{fig:evidence-dashboard}(b,c) 同时披露 B2 的长耗时尾部以及 A1/A2/A7 的接近零效应，避免用单一总体柱图隐藏边界。

\begin{figure}[htbp]
\centering
\includegraphics[width=0.98\textwidth]{generated/failure_case_qualitative.pdf}
\caption{由固定种子和同一规划代码生成的轨迹级边界诊断。上行延迟横穿样本中，B0 停车而 MIKU 选择 \texttt{pass\_before} 时窗并到达；下行预测含噪样本中，B0 在 $t=2.2$\,s 碰撞，而 MIKU 因有限域不可证而安全停车。红色带为真值障碍物占据，图例同时报告结果、同伦标签和证书状态。该图是算法级仿真诊断，不是实车或 Apollo/CyberRT 轨迹。}
...
各组件消融均按配对差报告，不预设每项都带来正收益。A6 同时关闭路径/ST 鲁棒膨胀与候选证书中的鲁棒误差管，其碰撞差为 \AblMikuVsASixCollisionDiff 个百分点（$p=\AblMikuVsASixCollisionMcNemarP$）；置信区间跨零的效果视为不确定。A7 的小幅变化不作显著性外推。

\subsection{滚动规划与联合参照}

滚动实验以 0.5\,s 为执行步长循环“规划--执行--更新预测”，每类 100 个种子，共 \ClosedCaseCount 个场景。MIKU 到达率为 \ClosedMikuSuccessPct\%，B0 为 \ClosedBZeroSuccessPct\%，配对提升 \ClosedMikuVsBZeroSuccessDiff 个百分点，95\% CI [\ClosedMikuVsBZeroSuccessCiLow,\ClosedMikuVsBZeroSuccessCiHigh]，McNemar $p=\ClosedMikuVsBZeroSuccessMcNemarP$。MIKU/B0 碰撞率分别为 \ClosedMikuCollisionPct\%/\ClosedBZeroCollisionPct\%，配对差不显著（$p=\ClosedMikuVsBZeroCollisionMcNemarP$）。
...
联合参照中，MIKU 与 B3 的到达率差为 \JointMikuVsBThreeSuccessDiff 个百分点，95\% CI [\JointMikuVsBThreeSuccessCiLow,\JointMikuVsBThreeSuccessCiHigh]，McNemar $p=\JointMikuVsBThreeSuccessMcNemarP$；该差异显著。由于 B3 使用不同粗网格和碰撞检查器，本结果仅作诊断，不宣称质量或计算优势。

\begin{figure}[htbp]
\centering
\includegraphics[width=0.62\textwidth]{generated/joint_search_scaling.pdf}
\caption{非平凡有限联合域的规模诊断。每层含两个可行空间带，并叠加一个动态横穿。空间分支的路径 QP 最优值提供可采纳下界，候选域与 QP 评估数的差异直接显示实际剪枝。}
\label{fig:joint-stress}
\end{figure}

图~\ref{fig:joint-stress} 将相互分离的空间冲突层从 1 增至 \StressMaxLayers。最大实例含 \StressSpatialCount 个空间同伦与 \StressDomainSize 个联合候选，搜索仅执行 \StressEvaluatedCount 次 QP 验收，三次重复的中位完整耗时为 \StressRuntimeMedian\,ms。该压力协议同时证明有限域证书和路径 QP 分支下界的实际剪枝；最坏复杂度仍随联合域指数增长，因此不作无条件实时性主张。

\subsection{外部 CommonRoad 审计}
\label{sec:commonroad}

为将公开基准来源与配对随机协议分开，本文固定 CommonRoad~\citep{althoff2017commonroad}
NGSIM Lankershim 的 4 个场景，并在隔离环境中以 $\Delta t=0.1$ s、40 步时域运行官方
reader、路线/参考线工具、Reactive Planner、solution writer 以及 Drivability Checker~\citep{pek2020drivability}。
官方竞品入口完成了全部 4 个 XML：其中 2 条轨迹通过官方动力学、碰撞、道路边界与目标检查，另有 2 次为规划器失败（表~\ref{tab:commonroad}）。

MIKU 侧结果分为两层报告。新增原生边界 runner 读取官方 planning problem，将成功轨迹转换为标准 KSState/PlanningProblemSolution，调用官方 evaluator，并保留规划失败；在这 4 个文件上 MIKU 真实产生 4 次 planner failure、0 条 valid solution，这是负结果而非外部优越性主张。其规划输入使用官方参考路线、全部动态障碍物的矩形形状与姿态轨迹：每个发布状态的四个车体角点被投影到 Frenet 坐标，并以时间插值的保守占用包络进入 ST 边界和连续安全证书。官方 lanelet 路由与 planning-problem goal 保持不变，但交通控制规则尚未由当前 Frenet planner 主动优化；因此这里报告的是语义范围明确的 CommonRoad 兼容性基准，不将未覆盖的规则策略扩写成 leaderboard 性能结论。

\begin{table}[htbp]
\centering
\scriptsize
\caption{固定 CommonRoad 外部审计；Native MIKU 行使用官方路由、逐状态形状/姿态占用包络、
solution/evaluator，但不解释为一般 leaderboard 分数。}
\label{tab:commonroad}
\begin{tabular}{lrrrr}
\toprule
协议 & $n$ & 有效/到达(\%) & 碰撞(\%) & 中位耗时(ms)\\
\midrule
官方 Reactive Planner & \CRReactiveScenarioCount & \CRReactiveValidCount/\CRReactiveValidPct & -- & --\\
Native MIKU boundary & \CRNativeMikuScenarioCount & \CRNativeMikuValidCount/\CRNativeMikuValidPct & -- & \CRNativeMikuMedianRuntime\\
适配器 B0 & \CRAdapterCaseCount & \CRBZeroSuccessPct & \CRBZeroCollisionPct & \CRBZeroMedianRuntime\\
适配器 MIKU & \CRAdapterCaseCount & \CRMikuSuccessPct & \CRMikuCollisionPct & \CRMikuMedianRuntime\\
\bottomrule
\end{tabular}
\end{table}
...
系统证据方面，仓库固定 Apollo Planning commit
\texttt{57460908954e3188f640a813d26180e862d62a5f}，并索引 Planning/CyberRT/Dreamview
fixture 与已有 runtime 产物。这些清单证明源码和资产来源；本次论文重建没有重新启动原生 Apollo 运行，因此本文数值仍属于算法级结果，不把它们表述为原生 Apollo runtime 性能。
